...
def ecg_clean(data_folder, subject, fs):
    """
    Genecleans ECG_clean for all sessions within a subject
    ...
    output:
    - clean_total: dictionary with keys being the session_id, and values being the ecg_clean
    ...
    #looping over all 50 sessions of subjects, computing its ecg_clean
    for session in range(len(ecg_total)):
        ...
        clean_total[f"session_{session:02d}"]=ecg_clean
    
    clean_total["Removed sessions"] = session_removed
    clean_total["# sessions removed"] = number_session_removed
        
    return clean_total

def process_subject(subject_id):
    """
    takes the subject, apply ect_clean function and save the resulting result into a file
    """
    # here adjust data_folder to the correct directory
    # processes 50 sessions of one subject and save the ecg_clean in pkl.gzip files
    data_folder = '100data'
    fs = 250
    output={}
    try:
        subject_data = ecg_clean(data_folder, subject_id, fs)
        output[f"{subject_id:05d}_subject"] = subject_data
        dataframe = pd.DataFrame(output)
        dataframe.to_pickle(f'ecg_clean_filtered/{subject_id:05d}_ecg_clean.pkl.gz', compression='gzip') 
